...
export function DocumentViewer({ fileId, title, onClose }: DocumentViewerProps) {
  ...
  return createPortal(
    ...
    >
      <section className="flex h-[100dvh] w-full flex-col overflow-hidden bg-muted shadow-2xl sm:h-[calc(100dvh-2rem)] sm:max-w-[1500px] sm:rounded-2xl sm:border">
        <header className="flex shrink-0 items-center justify-between gap-4 border-b bg-card px-4 py-3 sm:px-6">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-wide text-blue-600 dark:text-blue-400">
              Vista segura
            </p>
            <h2 id="document-viewer-title" className="truncate font-bold text-card-foreground">
              {title}
            </h2>
          </div>
          <div className="flex items-center gap-2">
            {url && (
              <a
                href={url}
                download={fileName}
                className="inline-flex h-10 items-center gap-2 rounded-xl border bg-background px-3 text-sm font-semibold text-foreground hover:bg-muted"
              >
            ...
            <button
              ...
              className="flex size-10 items-center justify-center rounded-xl bg-foreground text-background opacity-90 hover:opacity-75"
            >
              <X className="size-5" />
            </button>
          </div>
        ...
        <div className="relative flex h-0 flex-1 items-center justify-center overflow-hidden bg-muted p-3 sm:p-5">
          {!url && !error && (
          ...
          {error && (
            <div className="max-w-md rounded-2xl border border-red-200 bg-card p-6 text-center shadow-sm dark:border-red-900">
              <FileWarning className="mx-auto size-10 text-red-500" />
              <p className="mt-3 font-semibold text-card-foreground">No pudimos abrir el archivo</p>
          ...
          {url && mimeType.startsWith('image/') && !browserUnsupportedImage && (
            <div className="relative h-full w-full overflow-auto rounded-xl">
              ...
              <div className="fixed bottom-4 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1 rounded-2xl border bg-card/95 p-1.5 text-card-foreground shadow-xl backdrop-blur sm:bottom-7">
                <button
                  type="button"
                  onClick={() => setZoom((value) => Math.max(0.5, value - 0.25))}
                  disabled={zoom <= 0.5}
                  aria-label="Alejar"
                  className="flex size-10 items-center justify-center rounded-xl hover:bg-muted disabled:opacity-35"
                >
