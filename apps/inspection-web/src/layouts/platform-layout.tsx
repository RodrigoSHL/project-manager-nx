import { useState } from 'react';
import {
  ArrowLeft,
  Building2,
  LogOut,
  Menu,
  ShieldCheck,
  Zap,
} from 'lucide-react';
import { Link, NavLink, Outlet, useNavigate } from 'react-router-dom';
import { Button } from '../components/ui/button';
import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetTitle,
} from '../components/ui/sheet';
import { usePlatformAuth } from '../features/platform/platform-auth-context';

function PlatformSidebar({ onNavigate }: { onNavigate?: () => void }) {
  const auth = usePlatformAuth();
  const navigate = useNavigate();

  function logout() {
    auth.logout();
    onNavigate?.();
    navigate('/platform/login', { replace: true });
  }

  return (
    <>
      <div className="border-b border-white/10 px-5 py-5">
        <div className="flex items-center gap-3">
          <span className="grid size-9 place-items-center rounded-lg bg-amber-400 text-slate-950">
            <Zap className="size-5" fill="currentColor" />
          </span>
          <div>
            <p className="font-semibold text-white">GridAssets</p>
            <p className="text-xs text-amber-300">Plataforma</p>
          </div>
        </div>
      </div>

      <div className="px-5 py-5">
        <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">
          <ShieldCheck className="size-4" /> Control global
        </div>
        <p className="mt-2 text-xs leading-5 text-slate-500">
          Esta área administra clientes y no depende del tenant operativo.
        </p>
      </div>

      <nav className="flex flex-1 flex-col gap-1 px-3" aria-label="Plataforma">
        <NavLink
          to="/platform/tenants"
          onClick={onNavigate}
          className={({ isActive }) =>
            `flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm transition-colors ${
              isActive
                ? 'bg-white/10 font-medium text-white'
                : 'text-slate-300 hover:bg-white/5 hover:text-white'
            }`
          }
        >
          <Building2 className="size-5" /> Clientes
        </NavLink>

        <Link
          to="/dashboard"
          onClick={onNavigate}
          className="mt-3 flex min-h-11 items-center gap-3 border-t border-white/10 px-3 pt-3 text-sm text-slate-300 hover:text-white"
        >
          <ArrowLeft className="size-5" /> Volver a la operación
        </Link>
      </nav>

      <div className="border-t border-white/10 p-3">
        <div className="mb-2 px-3 py-2">
          <p className="truncate text-sm font-medium text-white">
            {auth.session?.user.name}
          </p>
          <p className="truncate text-xs text-slate-400">
            {auth.session?.user.email}
          </p>
        </div>
        <button
          type="button"
          onClick={logout}
          className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-sm text-slate-300 transition-colors hover:bg-white/5 hover:text-white"
        >
          <LogOut className="size-5" /> Cerrar sesión administrativa
        </button>
      </div>
    </>
  );
}

export function PlatformLayout() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  return (
    <div className="min-h-screen bg-slate-100 text-slate-950">
      <aside className="fixed inset-y-0 left-0 hidden w-72 flex-col bg-slate-950 md:flex">
        <PlatformSidebar />
      </aside>

      <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
        <SheetContent side="left" className="w-72 bg-slate-950 p-0">
          <SheetTitle className="sr-only">
            Administración de plataforma
          </SheetTitle>
          <SheetDescription className="sr-only">
            Navegación global de clientes
          </SheetDescription>
          <div className="flex h-full flex-col">
            <PlatformSidebar onNavigate={() => setIsMenuOpen(false)} />
          </div>
        </SheetContent>
      </Sheet>

      <div className="md:pl-72">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-slate-200 bg-white px-4 md:px-8">
          <Button
            variant="ghost"
            size="icon"
            className="md:hidden"
            aria-label="Abrir menú"
            onClick={() => setIsMenuOpen(true)}
          >
            <Menu />
          </Button>
          <div>
            <p className="text-sm font-semibold text-slate-900">
              Administración de plataforma
            </p>
            <p className="text-xs text-slate-500">Gestión global de clientes</p>
          </div>
        </header>

        <main className="mx-auto w-full max-w-7xl p-4 md:p-8">
          <Outlet />
        </main>
      </div>
    </div>
  );
}
