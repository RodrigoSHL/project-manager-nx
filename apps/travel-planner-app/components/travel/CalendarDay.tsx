...
export function CalendarDay({
  ...
  const transportActivities = activities.filter(a => a.type === 'flight' || a.type === 'train')
  const otherActivities = activities.filter(a => a.type !== 'flight' && a.type !== 'train')
  const maxVisible = 2
  const allVisible = [...transportActivities, ...otherActivities].slice(0, maxVisible)
  ...
  return (
    <div
      className={cn(
        ...
        isCurrentMonth ? 'bg-card border-border' : 'bg-muted/40 border-border/50',
    ...
    >
      ...
      {countries.length > 0 ? (
        <div className={cn(
          ...
          <button
            onClick={e => { e.stopPropagation(); onEditTravelDay(date) }}
      ...
      <div className="flex items-center justify-between px-2 pt-1.5 pb-1">
        ...
        <button
          onClick={e => { e.stopPropagation(); onAddActivity(date) }}
      ...
      <div className="px-1.5 pb-1.5 flex flex-col gap-0.5 flex-1">
        ...
        {overflow > 0 && (
          <span className="text-[10px] text-muted-foreground pl-1">+{overflow} más</span>
        )}
      </div>
    </div>
  )
